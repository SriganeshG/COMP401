...
public class A4JediTest {
	...
	public void testThreshold() {
		...
		assertTrue("Expected threshold transform to produce black.", comparePixels(res, black));
		res = xform.transform(p2);
		assertTrue("Expected threshold transform to produce white.", comparePixels(res, white));
	}
	...
	public void testGamma() {
		...
		Pixel p2 = new ColorPixel(Math.pow(0.2, (1.0 / 0.4)), Math.pow(0.4, (1.0 / 0.4)), Math.pow(0.7, (1.0 / 0.4)));

		Pixel res = xform.transform(p1);
		assertTrue("Gamma correct transform doesn't seem to produce correct pixel", comparePixels(res, p2));
	}
	...
	public void testTransformedPicture() {
		...
		Pixel ga = new ColorPixel(Math.pow(0.2, (1.0 / 1.3)), Math.pow(0.3, (1.0 / 1.3)), Math.pow(0.4, (1.0 / 1.3)));
		Pixel gb = new ColorPixel(Math.pow(0.7, (1.0 / 1.3)), Math.pow(0.7, (1.0 / 1.3)), Math.pow(0.2, (1.0 / 1.3)));
		Pixel gc = new ColorPixel(Math.pow(0.3, (1.0 / 1.3)), Math.pow(0.3, (1.0 / 1.3)), Math.pow(0.3, (1.0 / 1.3)));
		Pixel gd = new ColorPixel(Math.pow(0.7, (1.0 / 1.3)), Math.pow(0.7, (1.0 / 1.3)), Math.pow(0.7, (1.0 / 1.3)));

		Picture gamma_transformed = p.transform(gamma);
		Picture thresh_transformed = p.transform(thresh);
		//Pixel test = gamma_transformed.getPixel(0, 0);
